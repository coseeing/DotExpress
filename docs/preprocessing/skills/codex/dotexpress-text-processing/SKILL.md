...
1. Inspect the requested behavior and any legacy implementation completely.
...
4. Write or update one self-contained `.py` file with `main(text: str) -> str`. Use `apply_patch` for repository edits.
5. Verify representative Chinese, English, punctuation, empty-string, Unicode, and `\n`/`\r\n` cases.
