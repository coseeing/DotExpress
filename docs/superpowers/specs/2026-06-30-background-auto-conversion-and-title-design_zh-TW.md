...
背景轉換不能只用文件名稱判斷結果是否可回寫，必須區分「文字版本」與「轉換工作」兩種識別。
...
`text_version``代表某份文件的文字內容版本。

建議做法：

- 每份文件在記憶體中維護一個單調遞增整數。
- 每次文件文字內容被保存、重新排程背景轉換，或被手動送進前景轉譯時，都使用當下的最新 `text_version`。
- 若文件文字再次變更，版本號必須遞增。

`text_version` 的用途不是保證畫面上的 source text 與 braille result 永遠同步，而是避免過時的背景或前景轉換結果在完成後被當成最新版本回寫。

`text_version` 不需要持久化到 `.dep` 或其他磁碟資料中。

理由：

- 它只用來保護單次程式執行期間的非同步 job 回寫。
- 重新啟動後，舊 job 已不存在，無須延續先前 session 的版本號。
- 文件重新讀入時，可從初始值重新開始，例如 `0`。

### `job_id`

`job_id` 代表一次具體的轉換工作，不代表文字內容版本。

建議做法：

- `job_id` 使用單機唯一值，例如遞增整數或 `uuid4`。
- 每次建立背景或前景轉換工作時，都產生新的 `job_id`。

`job_id` 的用途是：

- 取消特定轉換工作
- 追蹤目前 running / queued 的工作
- 在完成 callback 時辨識是哪一次工作返回結果

### Acceptance Rule

結果是否可接受，應以該 job 的 `text_version` 是否仍匹配該文件目前最新的 `text_version` 為準；`job_id` 只負責識別工作本身，不應拿來代替文字版本判斷。
